...
import { ShieldCheck, Lock, AlertCircle, CheckCircle, Key } from 'lucide-react';

const API_BASE = 'http://localhost:8001/api';

export function AdminLogin({ onLoginSuccess }) {
  const { login } = useAuth();
  const [email, setEmail] = useState('admin@rebid.ai');
  ...
  const [resetEmail, setResetEmail] = useState('admin@rebid.ai');
    ...
    <div className="login-screen">
      <div className="login-card">
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <h1>Admin Portal</h1>
          <p className="text-muted" style={{ marginTop: '4px' }}>Compliance, Governance & Audit Trail Console</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="btn btn-primary" style={{ width: '100%', backgroundColor: '#111827', marginTop: '8px' }} disabled={loading}>
            <Lock size={16} />
          ...
          <div className="card" style={{ width: '400px', margin: 0 }}>
            ...
            <form onSubmit={handleResetSubmit}>
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '16px' }}>
                ...
                <button type="submit" className="btn btn-primary" style={{ backgroundColor: '#111827' }}>Update Password</button>
              </div>
            </form>
          </div>
        </div>
